...
**Коротко:**
- `app.py` — клас `Figure` з пропертями та тестовою функцією `test_app_triangle`.
...
- Створили/розробили файли: `app.py`, `test.py`, `validate_input.py`, `name_example.py`, `.gitignore`, `requirements.txt`, `.coveragerc`.
...
Якщо потрібно виділити текст, можна використати спеціальне позначення:

<< Текст, який потрібно виокремити (наприклад, короткий коментар або підсумок) >>
...
HTML-звіт буде у `htmlcov/index.html` — відкрийте його локально в браузері.

Якщо у вас багато графічних файлів (скріншотів), помістіть їх в окрему папку, наприклад `pictures/`, і використовуйте локальні шляхи при вставці у звіт.
...
Файли проекту у корені: `app.py`, `test.py`, `validate_input.py`, `name_example.py`, `requirements.txt`, `README.md`, `htmlcov/` (звіти). 
